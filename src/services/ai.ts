import { handleOpenAIError, logError, withRetry } from "../utils/errors";
...
export function createAIService(config: OpenAIConfig): AIService {
  ...
  return {
    ...
    async *generateChatCompletion(
      ...
      console.log(
        `💬 Starting chat completion with ${messages.length} messages...`
      );
      ...
      try {
        ...
        for await (const chunk of completion) {
          try {
            ...
            console.error(
              "🔥❌ Error processing chat completion chunk:",
              chunkError
            );
        ...
        console.log(
          `✅ Chat completion finished, streamed ${tokenCount} content chunks`
        );
      } catch (error) {
        const pluginError = handleOpenAIError(error);
