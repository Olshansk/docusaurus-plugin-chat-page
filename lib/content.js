...
async function generateEmbeddings(chunks, openAIConfig, embeddingConfig = {}) {
    ...
    console.log(`🔮 Generating embeddings for ${totalChunks} chunks in batches of ${batchSize}...`);
...
async function loadContent(context) {
    ...
    const embeddingCache = { ...constants_1.DEFAULT_EMBEDDING_CACHE_CONFIG, ...options?.embeddingCache };
    const embeddingConfig = { ...constants_1.DEFAULT_EMBEDDING_CONFIG, ...options?.embedding };
