...
async function loadContent(context) {
    ...
    if (embeddingCache.enabled) {
        try {
            ...
            console.log(`[CACHE] File read successfully. Strategy: ${embeddingCache.strategy}`);
            ...
            if (embeddingCache.strategy === "manual") {
                ...
                if (cacheJson.chunks &&
                    ...
                    console.warn(`[CACHE] Validation failed: chunks=${!!cacheJson.chunks}, isArray=${Array.isArray(cacheJson.chunks)}, metadata=${!!cacheJson.metadata}`);
                }
            }
            ...
            if (embeddingCache.strategy === "manual") {
                throw (0, errors_1.createError)(errors_1.ErrorType.FILE_SYSTEM, `Cache file not found at ${cacheFullPath}`, "Manual embedding cache strategy requires a valid cache file. Please generate embeddings manually.", {
                    emoji: "📋❌",
                    retryable: false,
                });
            }
    ...
    if (cacheValid && cacheData) {
        console.log(`[CACHE] Valid, returning cached data with ${cacheData.chunks.length} chunks`);
        return cacheData;
    }
    console.log(`[CACHE] Not valid (cacheValid=${cacheValid}, hasData=${!!cacheData}), proceeding with embedding generation`);
